...
void count_from_enpassant(position_node *root) {
  ...
  position_state s = {0};
  // CASE 0: there's no enpassant square
  // in which case there are currently 64 unoccupied squares
  s.free_squares = NUM_SQUARES;
  // we deal with side1 count_from_pawns for no real reason other than we like
  // to flip between side0 and side1 and this is currently side0
  count_from_pawns(&s, root->children[NO_ENPASSANT], 1, 0);
  mpz_add(root->num_positions, root->num_positions,
          root->children[NO_ENPASSANT]->num_positions);

  // CASE 1: enpassant, enpassant square not on edge file, and only one adjacent
  // opposition pawn case
  s.fixed_pawns[0] = 1;
  s.fixed_pawns[1] = 1;
  ...
  // enpassant square on edge file case
  ...
  // enpassant with two adjancent opposition pawns case
  s.fixed_pawns[1] = 2;
...
// characteristic of a board, where a board is represented by a leaf. The 1st
// level represents enpassant, and the first scenario is no enpassant square
// which is then the first child of root. We use a depth-first recursive
// approach so every board with no enpassant square is considered before boards
// with enpassant pawns. And every board with no enpassant square and 1 side0
// pawn is considered before boards with no enpassaant pawn and 2 side0 pawns
// etc. We choose the level significances s.t. we represent any board at most
// once, and reduce the number of boards which aren't reacahble without
// considering details which might be where exactly pieces on the board. For
// instance, the general pieces level doesn't consider where exactly pieces are
// placed and we minimise unreachable positions by considering the maximum
// number of pieces each side can have depending on how many pieces must be
