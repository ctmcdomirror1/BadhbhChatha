...
// I'm quite happy with this project but there's a lot of work that would
// need to be done to get it to an operational state. To properly close the
// loop a position prover as opposed to filters for proving unreachability
// would be desirable. Brute forcing if you have a strong machine or access
// to a cluster is viable but putting more thought into proving positions
// reachable and doing so on an average machine would be cooler imo.
