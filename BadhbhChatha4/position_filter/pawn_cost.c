...
// I would take a very different approach to the pawn filter. As there are more
// complex interactions between pawns and check conditions than I knew about
// when writing this file I took a simple brute force approach. What it really
// wants is a solver of some sort where we can enforce bipartite matching as
// well as many other check-related constraints
...
char validate_pawn_cost(position p) {
  ...
  max_costs.side0 = NUM_CHESSMEN_PSIDE_LESS_KING - num_capturable_chessmen[1];
  max_costs.side1 = NUM_CHESSMEN_PSIDE_LESS_KING - num_capturable_chessmen[0];
