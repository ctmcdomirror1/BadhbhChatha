...
# Permutations of generic pieces can have different costs due to each
# type having different amounts of base pieces - especially because we
# don't include rooks with castling rights as 'base pieces' here, even though
# they are of course. Since we can have at most 2 base pieces, we only care
# for 'p' in generic pieces whether 'p' is 0, 1 or >= 2. We can therefore map
# generic piece sets to covered sets via cs = [min(p, 2) for p in gp], consider
# all permutations of covered sets and their associated costs.
NUM_COVERED_SETS = 15
