...
// Could-Have-Done:
// There's certain notes in this file where the cases aren't implemented.
// Could do with more tests for each case
...
checking_info validate_checks_side1(position p) {
  ...
  if (num_checking_chessmen == 1) {
    // NOTE: Should look for escape square or discovery
    //
    if (checking_knights) {
    ...
    if (cp.bit != NUM_SQUARES) {
      ...
      // NOTE: or via pawn discovery

      // NOTE: now there has to be an escape square where we weren't
      // checking the king previously, and we have to communicate that this
      // requires a capture. The escape square can potentially be on the same
      // line as checking line, given there's space on the line.
