...
position retrieve_position_from_rng(position_node* root, mpz_t index)
{
    ...
    for (int i = 0; i < NUM_SIDES; i++)
    {
        ...
        {
            num_chessmen = pass_generic(&root, index, &occupied_squares, &p.sides[i].pieces[j]);
    ...
    for (int i = 0; i < NUM_SIDES; i++)
    {
        coveredSet_indices[i] = fr_coveredSet_index[nfr[i]][covered_sets[i][0]][covered_sets[i][1]]
    ...
    {
        permutations[i]
            = fr_coveredSetIndex_permIndex_perm[nfr[i]][coveredSet_indices[i]][pi.indices[i]];
    }
