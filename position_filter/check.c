...
checking_info validate_checks_side1(position p) {
  ...
  for (int i = 0; i < MOORE_NEIGHBOURHOOD_SIZE; i++) {
    ...
    // A queen can check in any line, but depending on whether the direction is
    // orthogonal or diagonal the other potentially checking piece is a rook or
    // bishop, respectively.
    char other_piece_type = 2 * ((i + 1) % 2);
  ...
  // Every double check has to be through discovery, but two bishops can't cover
  // each other because if a just-promoted pawn discovers a checking bishop,
  // that pawn would have been previously checking the 8th rank king.
  ...
  if (p.sides[0].fixed_rooks) {
    // Rook hasn't moved, and opposition king can't move into check
